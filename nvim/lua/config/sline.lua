...
local primary_hl = function(self)
	return { bg = self.mode_color, fg = colors.fg1, bold = true }
end
local secondary_hl = function(self)
	return { bg = self.bg_color, fg = self.mode_color }
end
local tertiary_hl = function(self)
	return { bg = self.bg_color, fg = colors.fg2 }
end
local delim_hl = function(self)
	return { bg = colors.bg, fg = self.bg_color }
end
...
local GitSignsBlock = {
	...
	},
	{
		...
		provider = function(self)
			return ' 柳' .. self.status_dict.changed
		end,
	},
	Space,
	{
...
FileNameBlock = utils.insert(FileNameBlock, Space, FileIcon, FileName, Space)
...
local FileSize = {
	provider = function(self)
		return get_fsize(self.filename) .. ' '
	end,
	condition = function(self)
		return vim.fn.glob(self.filename) ~= ''
	end,
	hl = tertiary_hl,
}
